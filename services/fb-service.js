...
module.exports = {
    sendPassThread: async function(senderID) {
        const url = `https://graph.facebook.com/v2.6/me/pass_thread_control?access_token=${process.env.FB_PAGE_TOKEN}`
        const body = {
            recipient: {
                id: senderID
            },
            target_app_id: process.env.FB_PAGE_INBOX_ID // ID in the page inbox setting under messenger platform
        }
    ...
    handleEcho: function(messageId, appId, metadata) {
        ...
        // log.magenta("Received echo for message %s and app %d with metadata %s", messageId, appId, metadata);
    },
    ...
    receivedMessageRead: function(event) {
        ...
        // log.yellow("Received message read event for watermark %d and sequence " +
    ...
    receivedAccountLink: function(event) {
        ...
        log.cyan("Received account link event with for user %d with status %s " +
    ...
    receivedDeliveryConfirmation: function(event) {
        ...
        //         log.green(`Received delivery confirmation for message ID: ${messageID}`);
        //     });
        // }

        // log.green(`All message before ${watermark} were delivered.`);
    },
    ...
    receivedAuthentication: function(event) {
        ...
        log.black("Received authentication for user %d and page %d with pass " +
    ...
    callSendAPI: async function(messageData) {
        try {
            let url = `https://graph.facebook.com/v3.2/me/messages?access_token=${process.env.FB_PAGE_TOKEN}`
            let response = await axios.post(url, messageData)
            let recipientId = response.data.recipient_id;
            let messageId = response.data.message_id;
            // if (messageId) {
            //     log.green(`Successfully sent message with id ${messageId} to recipient ${recipientId}`);
            // }
            log.cyan(`Successfully called Send API for recipient ${recipientId}`);
        } catch (error) {
